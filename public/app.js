...
async function jsonRequest(url,options={}){if(url!=='/api/catalog'||(options.method&&options.method!=='GET'))throw new Error('AI services are not included in this preview.');const response=await fetch(url,{cache:'no-store',...options});if(response.status===204)return{};let body;try{body=await response.json();}catch{const error=new Error(`The local service returned an unreadable response (HTTP ${response.status}).`);error.status=response.status;throw error;}if(!response.ok){const error=new Error(typeof body.error==='string'?body.error:body.error?.message||body.message||`The request failed (HTTP ${response.status}).`);error.status=response.status;error.errorCode=body.errorCode;error.recovery=body.recovery;throw error;}return body;}
...
function setView(mode){if(!['original','programmatic'].includes(mode))return;
...
async function checkHealth(){state.health={proxyReachable:false};$('proxyStatus').textContent='Local non-AI preview. No provider connected.';$('refreshConnection').disabled=true;updateDisabledState();}
...
function jobDescription(job){
  ...
  if(job.status==='queued')return'The complete submitted design is queued for the local proxy.';
...
async function generateAI(){throw new Error('AI generation is not included in this candidate.');
